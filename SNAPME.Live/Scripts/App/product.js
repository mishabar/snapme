app.controller('ProductController', function ($scope, $timeout, $interval, $filter, $mdDialog, productService) {
    $.loading = true;
    $scope.product = {};
    ...
    $scope.init = function(productName) {
        productService.getProduct(productName).then(
            function (response) {
    ...
    $scope.iJoined = function () {
        return $scope.product != null && $scope.product.snaps != null && $filter('filter')($scope.product.snaps, window._me, true).length == 1;
    }
